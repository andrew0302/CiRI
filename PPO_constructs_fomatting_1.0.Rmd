...
rm(
  questions,
  Entrskills_vars, Entrskills_df,
  LoC_vars, LoC_df,
  TfA_vars, TfA_df,
  perskills_vars, perskills_df, 
  NFA_vars, NFA_df,
  intentions_vars, intentions_df)
``` 

```{r}
T0_constructs_df <- data.frame(lapply(T0_constructs_df, function(x) {
                  gsub("Helemaal niet mee eens", 0, x)
              }))

T0_constructs_df <- data.frame(lapply(T0_constructs_df, function(x) {
                  gsub("Niet mee eens", 1, x)
              }))
