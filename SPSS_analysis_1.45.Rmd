...
```
```{r income_df_visualization, include=TRUE, fig.cap="Figure 4: Self-reported Net Income in 2018-2019 Euros at two time-points."}

set.seed(321)
income_long$income <- jitter(income_long$income, amount=.01)

income_long %>%
  filter(time != "T1") %>%
  ggplot(aes(x=time, y=income,group=participant_ID, fill=time)) +
    
   geom_point(
     data = income_long %>% filter(time=="T0"),
     color = ciri_colors[1],
     alpha = 0.7,
     #position = position_jitter(w = 0.075, h=0.2),
     show.legend = FALSE
     ) +
  
   geom_point(
     data = income_long %>% filter(time=="T2"),
     color = ciri_colors[2],
     alpha = 0.7,
     position = position_jitter(w = 0.075, h=0.2),
     show.legend = FALSE
